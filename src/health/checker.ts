...
function readProjectPackageJson(
  ...
  try {
    return JSON.parse(fs.readFileSync(packageJsonPath, "utf-8")) as Record<
      string,
      unknown
    >;
  } catch {
...
type LintConfigCheck =
  | {
      ...
      fileName: string;
      fullPath: string;
      propertyName: "ignores";
    }
  | {
      tool: "Oxlint";
      fileName: string;
      fullPath: string;
      propertyName: "ignorePatterns";
    };
...
export async function main(
  ...
  const installCommand = getProjectInstallCommand(
    projectRoot,
    projectPackageJson,
  );
  ...
  try {
    ...
    const tsPkgPath = path.resolve(
      path.dirname(tsserverPath),
      "..",
      "package.json",
    );
    ...
    const hasDeclaredTs = hasDeclaredDependency(
      projectPackageJson,
      "typescript",
    );
    fail(
  ...
  const hasPluginMcp =
    fs.existsSync(pluginMcpPath) &&
    fs.existsSync(path.join(toolDir, ".claude-plugin/plugin.json"));
  const projectCodexConfig = readProjectCodexConfig(projectRoot);
  const hasProjectCodexRegistration =
    projectCodexConfig !== null &&
    hasCompleteCodexTypegraphRegistration(projectCodexConfig);
  ...
  if (process.env.CLAUDE_PLUGIN_ROOT) {
    ...
    if (hasProjectCodexRegistration) {
      ...
      const issues: string[] = [];
      if (!hasSection)
        issues.push("[mcp_servers.typegraph] section is missing");
      if (!hasCommand) issues.push("command is missing");
      if (!hasArgs) issues.push("args are missing");
      if (!hasTsxLauncher)
        issues.push("command should point to tsx or args should include 'tsx'");
    ...
    if (fs.existsSync(mcpJsonPath)) {
      try {
        ...
        if (tsNav) {
          ...
          const hasArgs =
            Array.isArray(tsNav.args) && tsNav.args.includes("tsx");
          const hasEnv =
            tsNav.env?.["TYPEGRAPH_PROJECT_ROOT"] &&
            tsNav.env?.["TYPEGRAPH_TSCONFIG"];
          if (hasCommand && hasArgs && hasEnv) {
            ...
            if (!hasEnv)
              issues.push(
                "env should set TYPEGRAPH_PROJECT_ROOT and TYPEGRAPH_TSCONFIG",
              );
            fail(
      ...
      // Check for OpenCode or MiMo Code registration
      const opencodePath = path.resolve(projectRoot, "opencode.json");
      const mimocodePath = path.resolve(projectRoot, "mimocode.json");

      if (fs.existsSync(opencodePath)) {
        try {
          const config = JSON.parse(fs.readFileSync(opencodePath, "utf-8"));
          if (config.mcp?.typegraph) {
            pass("MCP registered in opencode.json");
          } else {
            fail(
              "MCP entry 'typegraph' not found in opencode.json",
              "OpenCode MCP registration is incomplete",
            );
          }
        } catch {
          fail("Failed to parse opencode.json", "Check JSON syntax");
        }
      } else if (fs.existsSync(mimocodePath)) {
        try {
          const config = JSON.parse(fs.readFileSync(mimocodePath, "utf-8"));
          if (config.mcp?.typegraph) {
            pass("MCP registered in mimocode.json");
          } else {
            fail(
              "MCP entry 'typegraph' not found in mimocode.json",
              "MiMo Code MCP registration is incomplete",
            );
          }
        } catch {
          fail("Failed to parse mimocode.json", "Check JSON syntax");
        }
      } else {
        fail(
          "No MCP registration found",
          `Create ${codexConfigPath} with [mcp_servers.typegraph], .claude/mcp.json, opencode.json, or mimocode.json with typegraph server registration`,
        );
      }
    }
  }
  ...
  if (fs.existsSync(toolNodeModules)) {
    ...
    if (missing.length === 0) {
      ...
      fail(
        ...
        `Run \`cd ${toolRelPath} && npm install\``,
      );
    }
  } else {
    fail(
      "typegraph-mcp dependencies not installed",
      `Run \`cd ${toolRelPath} && npm install\``,
    );
  }
  ...
  try {
    ...
    const { ResolverFactory } = await import(
      oxcResolverReq.resolve("oxc-resolver")
    );
  ...
  try {
    let buildGraph: (
      ...
      graph: { files: Set<string>; forward: Map<string, unknown[]> };
    }>;
    try {
      ...
      ({ buildGraph } = await import("../core/graph/index.js"));
    }
    ...
    if (graph.files.size > 0 && edgeCount > 0) {
      pass(
        `Module graph: ${graph.files.size} files, ${edgeCount} edges [${elapsed}ms]`,
      );
  ...
  if (fs.existsSync(gitignorePath)) {
    ...
    const parentDir = toolIsEmbedded
      ? path.basename(path.dirname(toolDir))
      : null;
    const ignoresParent =
      parentDir &&
      lines.some(
        (l: string) =>
          l === `${parentDir}/` || l === parentDir || l === `/${parentDir}`,
      );

    if (!ignoresParent && !ignoresClaude) {
      pass(
        ".gitignore does not exclude .claude/" +
          (parentDir ? ` or ${parentDir}/` : ""),
      );
    } else {
